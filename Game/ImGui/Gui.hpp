...
struct LoggerWindow
{
	...
	void Log(const char* text) {
		string log_text = "[LOG] " + string(text) + "\n";
		char* char_arr = &log_text[0];
		AddLog(char_arr);
	}
...
struct MainWindow
{
	...
	MainWindow(LoggerWindow *Logger) {
		logger = Logger;
	}
...
inline LoggerWindow Logger = LoggerWindow();
inline MainWindow GuiWindow = MainWindow(&Logger);